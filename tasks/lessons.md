...
102. - [altmark/infrastructure]: **n8n in Docker-in-WSL2 on Windows has several deployment-killers — fix all before trusting a schedule.** (1) WSL2 auto-terminates when idle, killing Docker+n8n. `vmIdleTimeout=-1` in `C:\Users\[user]\.wslconfig` is NOT reliable across all Windows builds — the dependable fix is a **keepalive scheduled task that pings WSL every ~5 minutes** to hold the VM up; set `vmIdleTimeout=-1` too as belt-and-suspenders but don't depend on it. (2) WSL2/Docker/n8n don't auto-start on boot — register a scheduled task (`schtasks /create /tn "Start n8n" /tr start-n8n.bat /sc onlogon /rl highest`); the Startup folder is unreliable on Win11. (3) **Docker port mapping breaks when WSL restarts**, so the startup script must RECREATE the container (`docker stop` + `docker rm` + `docker run …`), NOT just `docker start n8n` — the named volume (`-v n8n_data:…`) preserves all workflows/credentials, so recreation is lossless. (4) WSL2 networking is isolated — port 5678 isn't reachable externally and the internal IP changes per reboot, breaking `netsh portproxy`. The definitive fix is to run **Tailscale inside WSL2** (see #104), not `tailscale serve` on the Windows host. Pair all of this with Windows auto-login (registry `AutoAdminLogon`) + BIOS "Restore on AC Power Loss = Power On" so it self-recovers after an outage; use ethernet, not WiFi.

103. - [altmark/infrastructure]: **Deploy n8n on a dedicated mini PC, never on the client's production server.** Client IT will block Docker/port/admin changes on machines running QuickBooks/file shares (ThreatLocker etc.). A $300-600 mini PC (Beelink/Mac Mini/NUC) removes every objection: nothing touches their server, fully reversible (unplug it). Build it fully at home — OS, Docker, n8n, Tailscale, reboot+idle tests, OAuth published — before delivery; on-site is just power + ethernet.

104. - [altmark/infrastructure]: **For Docker-in-WSL2, install Tailscale on BOTH the Windows host AND inside WSL2, and use the WSL2 Tailscale node for direct n8n access.** This is the definitive networking fix — the in-WSL2 Tailscale node reaches n8n on `localhost:5678` directly, sidestepping WSL2's port isolation entirely (no `netsh portproxy`, no internal-IP-changes-on-reboot, no `tailscale serve`). The earlier "install on Windows only + `tailscale serve --bg 5678`" advice is SUPERSEDED — it gave connection resets for external/LAN traffic. Keep Windows-side Tailscale too (runs as an auto-starting service for the host), but the WSL2 node is what serves n8n. Use the Tailscale MagicDNS hostname (`https://[host].ts.net`, auto Let's Encrypt cert), never a raw `100.x.x.x` IP, for OAuth redirect URIs — Google rejects raw IPs and plain HTTP-with-domain. Dashboard via `http://[host].ts.net:5678` works day-to-day; the HTTPS endpoint may render blank (n8n asset quirk) but OAuth callbacks over it work.
