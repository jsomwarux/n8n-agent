...
31. - [lead-qualification-demo]: **HubSpot HTTP Request nodes return a 401 with "credentials not found" message** even when using `authentication: "none"` with explicit Bearer header, if the placeholder token is invalid. For demo purposes, set `onError: continueRegularOutput` on the HubSpot node so downstream Gmail/Slack/Sheets nodes still fire. Give client a setup step to replace the placeholder with a real Private App token.

32. - [lead-qualification-demo]: **For multi-path IF workflows (hot/cold branches), duplicate the Sheets/summary nodes on each path** rather than trying to merge branches with a Merge node. Two separate Sheets append nodes targeting the same tab is cleaner and avoids Merge node item-count issues in n8n 2.10+.
