...
[pm-maintenance-triage]: n8n Code node sandbox blocks require(), fetch(), and axios — no HTTP calls possible from Code nodes. Use HTTP Request node for external API calls, but it requires a bound n8n credential (auth:none does not bypass credential validation). Workaround: create an httpHeaderAuth credential with the API key, bind it to the node. For demos without live API: rule-based Code node logic works end-to-end and proves the routing/dispatch pattern.
