...
33. - [lead-qualification-demo]: **`specifyBody: "string"` + `jsonBody` with `JSON.stringify(...)` expression does not evaluate nested object expressions in n8n** — the JSON.stringify wrapper causes n8n's expression engine to return an empty or partially-evaluated object, resulting in a HubSpot contact with all blank properties. Fix: use `specifyBody: "json"` + `jsonBody` with a plain object expression `={{ { "properties": { "email": $("Node").item.json.email, ... } } }}` — n8n serializes the returned object automatically. Never use JSON.stringify inside n8n expressions.
...
37. - [support-triage-demo]: **HTTP Request nodes require `sendBody: true` explicitly** — without this flag, the `jsonBody` expression is ignored entirely and no body is sent. n8n returns "JSON parsing failed" or "Bad request" from the target API. Always include `sendBody: true` alongside `specifyBody: "json"` and `jsonBody`. Root cause: the property is not inferred from the presence of `jsonBody`.
