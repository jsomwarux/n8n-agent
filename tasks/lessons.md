...
102. - [altmark/infrastructure]: **n8n in Docker-in-WSL2 on Windows has three deployment-killers — fix all before trusting a schedule.** (1) WSL2 auto-terminates when idle, killing Docker+n8n — add `C:\Users\[user]\.wslconfig` with `[wsl2]` + `vmIdleTimeout=-1`, then full reboot. (2) WSL2/Docker/n8n don't auto-start on boot — register a scheduled task (`schtasks /create /tn "Start n8n" /tr start-n8n.bat /sc onlogon /rl highest`) running `wsl -u root -- bash -c "service docker start && docker start n8n"`; the Startup folder is unreliable on Win11. (3) WSL2 networking is isolated — port 5678 isn't reachable externally and the internal IP changes per reboot, breaking `netsh portproxy`. Working fix: `tailscale serve --bg 5678` on the Windows host (persists across reboots). Pair with Windows auto-login (registry `AutoAdminLogon`) + BIOS "Restore on AC Power Loss = Power On" so it self-recovers after an outage; use ethernet, not WiFi.
...
104. - [altmark/infrastructure]: **Use the Tailscale MagicDNS hostname (not a raw IP) for remote n8n access and OAuth.** `tailscale serve --bg 5678` exposes an HTTPS endpoint with an auto-provisioned Let's Encrypt cert at `https://[host].ts.net`, which Google OAuth and browsers accept. Raw Tailscale IPs (100.x.x.x) and plain HTTP-with-domain are rejected by Google's redirect-URI rules. Dashboard access via `http://[host].ts.net:5678` works day-to-day; the HTTPS endpoint may render blank (n8n asset quirk) but OAuth callbacks over it work. Install Tailscale on the Windows host, not inside WSL2 (it runs as a service and auto-starts).
