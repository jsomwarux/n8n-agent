"""Stage 3: Cross-review deliberation — each model reviews its own + others' Stage 2 outputs."""
...
async def _call_model(client: httpx.AsyncClient, model_key: str, prompt: str, attempt: int = 1) -> dict:
    """Call a single model for deliberation. Returns {model_key, raw_text, parsed, error}."""
...
async def run(product: dict, stage2_results: dict[str, dict]) -> dict[str, dict]:
    """Run deliberation for all models that succeeded in Stage 2. Parallel execution.

    Returns: {claude: {model_key, raw_text, parsed, error}, ...}
    """
    ...
    # Failed Stage 2 models get marked as skipped in Stage 3
    for model_key in MODELS:
        if model_key not in eligible:
            results[model_key] = {
                ...
                "raw_text": None,
                "parsed": None,
                "error": f"Skipped — failed Stage 2",
            }
